...
namespace ProjEnv
{
    ...
    //jingz 右手坐标系空间。每个面像素左上角起点作为贴图UV原点
...
public:
        ...
        const Scene* scene,int curBounces)
    {
        ...
        const int sample_side = static_cast<int>(std::floor(sqrt(m_SampleCount)));
        ...
        for (int t = 0; t < sample_side; t++)
        {
            ...
            {
                double alpha = (t + range(gen)) / sample_side;
                double beta = (p + range(gen)) / sample_side;

                double phi = 2.0 * M_PI * beta;
                double theta = acos(2.0 * alpha - 1.0);

                Eigen::Array3d delta = sh::ToVector(phi, theta);
                const auto wi = Vector3f(delta.x(), delta.y(), delta.z());
                ...
                if (H > 0.0 && scene->rayIntersect(Ray3f(pos, wi.normalized()), its))
                {
                    ...
                    Point3f triangeIndex = its.tri_index;
                    Point3f hitPos = its.p;
                    Vector3f baryCenter = its.bary;

                    Normal3f hitNormal = Normal3f(normals.col(triangeIndex.x()) * baryCenter.x()
                        + normals.col(triangeIndex.y()) * baryCenter.y()
                    ...
                    for (int i = 0; i < SHCoeffLength; i++)
                    {
                        auto interpolateSH = directionTransportSHCoeffs->col(triangeIndex.x()).coeffRef(i) * baryCenter.x()
                            + directionTransportSHCoeffs->col(triangeIndex.y()).coeffRef(i) * baryCenter.y()
                            + directionTransportSHCoeffs->col(triangeIndex.z()).coeffRef(i) * baryCenter.z();
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shTransportVisibilityFunc = [&](double phi, double theta) -> double {
                ...
                else
                {
                    ...
                    if (H > 0.0 &&
                        !scene->rayIntersect(Ray3f(v, wi.normalized()))
                        )
                    {
                        return H;
                    }
            ...
            auto shCoeff = sh::ProjectFunction(SHOrder, shTransportVisibilityFunc, m_SampleCount);
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                ...
                {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (*inDirectionCoeffs)[j];
                }//for_j
